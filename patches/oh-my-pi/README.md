...
| Optional encrypted `gateway-health` probes and duplicate-response acknowledgement (commit four) | Split. The acknowledgement half is submitted as [PR #11561](https://github.com/can1357/oh-my-pi/pull/11561) from `alphastorm:contrib/collab-ack-settled-ui-response` (signed head `e479f38ce097b952357e85a6394b87090e011977` on upstream `c5ae5cb14f`): a targeted `ui-request-end` to a writable guest whose `ui-response` names an already-settled request. The `gateway-health` seed/probe seam is not submitted and remains a required carry; when #11561 lands, shrink commit four to that seam. |
| Vendored `collab-web` link-scheme hardening (`packages/collab-client/upstream/UPSTREAM.json`) | Submitted as [PR #11562](https://github.com/can1357/oh-my-pi/pull/11562) from `alphastorm:contrib/collab-web-link-scheme-normalization` (signed head `501cb3bd3302edf9ecabe47726d25d8d65850c90` on upstream `c5ae5cb14f`): `safeHref` resolves the scheme with the URL parser. Opened as a quiet hardening PR without an exploit narrative; the vendored patch stays until it lands. |
