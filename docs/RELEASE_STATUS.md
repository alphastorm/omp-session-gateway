...
| Repository check | **PASS** | `bun run check` passed handoff validation, four workspace typechecks, production web/client builds, 97 tests with 449 assertions across 18 files, and capability-leak scanning. Protocol/IPC/HTTP coverage includes strict boolean attention metadata and same-generation false-to-true-to-false ordering. Two Android-sized Playwright projects separately passed explicit foreground notification and worker-click behavior. |
...
| Desktop attention/notification browser automation | **PASS** | Chromium at `412 × 915` and `390 × 844` passed attention-first ordering, accessible View/Control states, no permission prompt on load, explicit permission, initial-baseline silence, false-to-true notification, true-to-true dedupe, false rearm, prompt-content canary exclusion, service-worker support handshake, and notification-click focus/open of `/`. This does not prove Android OS notification or lock-screen presentation. |
...
| Repository automated suite | **PASS** | Typechecks, production asset builds, 97 unit/integration tests with 449 assertions across 18 files, handoff checks, capability-leak scanning, and two Android-sized Playwright projects passed. The attention suite proves strict metadata-only transitions, stale-state clearing, volatile notification dedupe, and dashboard-only notification clicks. | Repeat from clean CI for every subsequent candidate. |
...
| Android PWA installation | **PARTIAL** | A physical Android user installed `v0.1.0-prealpha.3` from the real tailnet HTTPS origin, loaded the metadata directory, observed **Needs attention**, and completed notification opt-in. Both View and Control then returned to the directory because Android standalone launch did not preserve `window.opener`; `v0.1.0-prealpha.4` replaces that path with a same-document in-memory mount. | Retest the corrected candidate, capture device/Android/Chrome versions, and verify update/offline states. |
...
| Real View and Control behavior | **PARTIAL** | Real desktop browser clients proved View composer disabled, Control prompt delivery, shared transcript updates, and interrupt. The first physical Android installed-PWA attempt on `v0.1.0-prealpha.3` exposed same-window launch returning to the directory before either capability could mount; the corrected source has Android-sized no-popup same-document launch coverage. | Retest View and Control from the physical Android candidate and verify host-side view mutation rejection. |
...
| Android attention notification | **NOT RUN** | Android-sized desktop Chromium passed the browser and worker contracts without prompt/payload canaries. | On one allowlisted physical Android/Chrome device with one live dashboard tab, verify the warning and only permission prompt, foreground false-to-true notification, lock-screen text, dashboard-only tap, SSE reconnect, and lock/resume. Do not infer background or killed-browser delivery. |
| Existing OMP relay connectivity | **PARTIAL** | Real desktop View/Control/interrupt and reconnect passed through the default relay. An earlier eight-hour read-only client completed after 28,804 seconds with three phase transitions and the client still live. The signed `v0.1.0-prealpha.1` repeat exposed terminal guest handling of transient room replacement after approximately 2 hours 4 minutes. The signed `v0.1.0-prealpha.2` recovery rerun then completed 28,800 seconds, eight transitions, `finalPhase: "live"`, exit code 0, no restart, and a 720 KiB gateway RSS increase. | Repeat connectivity, lock/resume, network-change, and reconnect on physical Android before broadening the relay claim. |
