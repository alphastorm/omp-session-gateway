...
| Windows hosted lifecycle qualification | **PASS** | GitHub Actions run `29715110343` completed strict ACL tests, UTF-16 scheduled-task install/start, health/status, token rotation, authenticated graceful restart with PID replacement, active reinstall, and process-clean uninstall. |
...
| Windows host lifecycle | **PARTIAL** | GitHub Actions `windows-latest` run `29715110343` passed config/token ACL checks, scheduled-task install/start, token rotation, authenticated graceful PID replacement, active reinstall, and process-clean uninstall. | Repeat with a signed candidate artifact; qualify reboot/login persistence, diagnostics, upgrade, and rollback. |
...
| Configuration migration and rollback | **PARTIAL** | Active reinstall/PID replacement passed on macOS and Linux; configuration and token ownership were preserved. | Execute explicit forward migration and rollback with candidate artifacts on each advertised host. |
...
| Release signing, SBOM, and provenance | **BLOCKED** | A least-privilege, commit-pinned OIDC workflow now builds a deterministic SPDX 2.3 inventory and signs/attests every release artifact; local reproducibility passed. No hosted attestation exists because GitHub Actions is in partial outage. | Run a provenance-test tag after Actions recovers and verify GitHub attestations, Cosign bundles, checksums, SBOM, and immutable release assets. |
