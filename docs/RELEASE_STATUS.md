...
| Host-suspension recovery experiment | **PASS** | The exact patched OMP publisher and gateway were suspended beyond a five-second test TTL, then resumed gateway-first, publisher-first, and together. All three orders retained or republished the session within eight seconds; gateway-first and simultaneous resume exercised expiry, clean authenticated disconnect, mutual re-authentication, and full upsert. This finite missed-timer reproduction does not replace actual macOS sleep/wake qualification. |
| Dependency audit | **PASS** | `bun audit` reported no vulnerabilities for the recorded lockfile. |
| OMP patch application and lifecycle fixtures | **PASS** | Patch apply-check passed against the pristine pin; 36 controller/publisher/settings/session-ordering/slash-command tests—including the mutual-HMAC vector, fake-server withholding, legitimate exchange, deterministic daemon-restart recovery that rereads a rotated token before republishing, and explicit-token-path publication that preserves ambient XDG configuration—and the full coding-agent package typecheck passed. |
