...
| Android client | Installable HTTPS PWA through Tailscale Serve | Pixel 10 Pro, Android 17 build `CP2A.260705.006` (SDK 37), Chrome `150.0.7871.128` passed the corrected `v0.1.0-prealpha.4` installed-PWA View/Control/Back path, exactly-once retained response, foreground and lock-screen attention notification, dashboard-only notification tap, lock/resume, Wi-Fi/cellular transition, automatic relay reconnect, generation replacement with stale `409`, and TTL removal/republication | Physical interrupt, switch/branch/saved-session resume, explicit offline-shell behavior, distinct-identity denial, and deep URL/history/storage/cache inspection remain unqualified | None |
