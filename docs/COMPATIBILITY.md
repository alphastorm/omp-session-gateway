...
| Windows host | Current-user named pipe; current-user scheduled task | GitHub Actions `windows-latest` run `29715110343` passed strict config/token ACL checks, UTF-16 task installation, active health, atomic token rotation, authenticated graceful PID replacement, active reinstall, and process-clean uninstall | Hosted development-checkout evidence is not signed-artifact qualification; reboot/login persistence, upgrade/rollback, and a release-candidate run remain unqualified | None |
