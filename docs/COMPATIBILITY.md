...
| Android client | Installable HTTPS PWA through Tailscale Serve | **Current device baseline (2026-08-20):** the Pixel 10 Pro is now on Android 17 build `CP2A.260805.005` (SDK 37) with Chrome `151.0.7922.139`. All recorded Android scenario and qualification evidence that follows predates this current device/browser combination and must be re-run before being claimed for it. A capability probe on the real gateway origin found `isSecureContext === true`, `navigator.serviceWorker` with a non-null controller, Cache Storage, `PushManager`, `Notification.permission === "granted"`, `navigator.setAppBadge`, `navigator.clearAppBadge`, and `PublicKeyCredential`. These are platform-capability observations only: badge API availability makes [`TEST_PLAN.md`](TEST_PLAN.md) gate D.10's badge requirement achievable on this device, and `PublicKeyCredential` makes the deferred WebAuthn control-gating path feasible; neither path is qualified. **Historical scenario evidence:** Pixel 10 Pro, Android 17 build `CP2A.260705.006` (SDK 37), Chrome `150.0.7871.128` passed the corrected `v0.1.0-prealpha.4` installed-PWA View/Control/Back, exactly-once retained response, foreground and lock-screen attention notification, dashboard-only notification tap, lock/resume, Wi-Fi/cellular transition, automatic relay reconnect, generation replacement with stale `409`, and TTL removal/republication. The independently verified `v0.1.0-prealpha.5` artifact then auto-discovered three real OMP processes, cleared all cards after a silent Airplane-mode partition, and restored exactly three from a fresh snapshot without Refresh. On signed `v0.1.0-prealpha.7`, the user reported the instructed explicit-enable, closed-PWA background notification, and tap-to-current-Control flow working on their phone; fresh device/OS/browser versions and a named evidence artifact were not captured. | Cold offline navigation is intentionally unavailable because navigation is never service-worker cached. The current device/browser combination and both capability-gated paths remain unqualified. The background Push lock-screen, force-stop, stale-generation, network-change, and forbidden-sink matrix remains incomplete, as do physical interrupt, host-observed View mutation rejection, switch/branch/saved-session resume, process crash, distinct-identity denial, and deep URL/history/storage/cache inspection. | None |
