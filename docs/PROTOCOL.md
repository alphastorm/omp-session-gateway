...
It must never contain a capability, room key, write token, relay secret, publisher token, transcript, answer, tool output, or full path by default. `ask.requestId` is opaque routing metadata generated by the gateway, and `ask.since` is daemon receipt time. `ask.preview` and `optionCount` remain absent until an explicit bounded publisher contract supplies them.
