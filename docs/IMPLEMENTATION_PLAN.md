...
5. Background Push API notifications — implemented after the separate ADR-017 privacy/security design; physical Android qualification remains open.

Do not combine remaining optional features with the v1 security-critical OMP patch. Background
Push is isolated in gateway/PWA contracts and may not be called supported until its physical
Android privacy, delivery, stale-tap, force-stop, and network-transition matrix passes.
