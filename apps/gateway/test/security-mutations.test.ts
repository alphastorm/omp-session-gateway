...
const MUTATIONS: readonly Mutation[] = [
  {
    name: "WebAuthn enrollment commits after its grant expires during private staging",
    file: "apps/gateway/src/webauthn.ts",
    find: "await writePrivateTextFile(path, `${JSON.stringify(state)}\\n`, beforeCommit);",
    replace: "await writePrivateTextFile(path, `${JSON.stringify(state)}\\n`);",
    target: "apps/gateway/test/webauthn.test.ts",
    mustFail: "enrollment expiring during real private staging cannot commit a credential",
  },
  {
    name: "WebAuthn signature accepts a challenge the gateway never issued",
    file: "apps/gateway/src/webauthn.ts",
    find: "expectedChallenge: ceremony.challenge",
    replace: "expectedChallenge: () => true",
    target: "apps/gateway/test/webauthn.test.ts",
    mustFail: "signature verifier rejects tampering origin RP and user verification",
  },
  {
    name: "WebAuthn session remains active after its absolute deadline",
    file: "apps/gateway/src/webauthn.ts",
    find: "entry.expiresAt <= this.#now() || !this.identityAllowed(entry.identityKey)",
    replace: "false || !this.identityAllowed(entry.identityKey)",
    target: "apps/gateway/test/webauthn.test.ts",
    mustFail: "real signatures admit only an enrolled credential and absolute opaque cookies expire",
  },
  {
    name: "WebAuthn registration permits cross-origin embedding",
    file: "apps/gateway/src/webauthn.ts",
    find: "      rejectEmbedding(input.response);",
    replace: "      // embedding check removed",
    target: "apps/gateway/test/webauthn.test.ts",
    mustFail: "registration rejects missing presence verification embedded origin and wrong RP",
  },
  {
    name: "WebAuthn verifier does not require user verification",
    file: "apps/gateway/src/webauthn.ts",
    find: "requireUserVerification: true",
    replace: "requireUserVerification: false",
    target: "apps/gateway/test/webauthn.test.ts",
    mustFail: "signature verifier rejects tampering origin RP and user verification",
  },
  {
    name: "WebAuthn concurrent assertions reuse a stale public signature counter",
    file: "apps/gateway/src/webauthn.ts",
    find: "counter: credential.counter }",
    replace: "counter: 0 }",
    target: "apps/gateway/test/webauthn.test.ts",
    mustFail: "synced zero counters remain usable and concurrent public counters serialize",
  },
  {
    name: "WebAuthn launch returns a capability after asynchronous revocation",
    ...
    replace: "      });\n      if (resolution.status",
    target: "apps/gateway/test/webauthn.test.ts",
    mustFail: "logout immediately closes SSE and a resolved launch cannot cross revocation",
  },
  {
    name: "WebAuthn idle SSE omits the absolute session-expiry timer",
    file: "apps/gateway/src/webauthn.ts",
    find: "const timer = setTimeout(() => this.#invalidate(sessionDigest), SESSION_MS);",
    replace: "const timer = setTimeout(() => {}, SESSION_MS);",
    target: "apps/gateway/test/webauthn.test.ts",
    mustFail: "absolute session timer closes an idle SSE stream without a heartbeat",
  },
  {
    name: "WebAuthn SSE emits metadata before rechecking a live session",
    file: "apps/gateway/src/http.ts",
    find: "          if (!stillAuthorized()) return close();",
    replace: "          // per-event authorization removed",
    target: "apps/gateway/test/webauthn.test.ts",
    mustFail: "SSE refuses metadata emitted after session expiry before its timer runs",
  },
  {
