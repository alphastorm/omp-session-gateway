...
- Hosted Windows lifecycle qualification: GitHub Actions run `29715110343` passed config/token ACL checks, UTF-16 scheduled-task install/start, active health/status, atomic token rotation, authenticated graceful PID replacement, active reinstall, and process-clean uninstall.
...
- GitHub private vulnerability reporting and immutable releases are enabled. The commit-pinned OIDC release workflow signs and attests the archive, SBOM, and checksum manifest; hosted execution remains blocked by the current GitHub Actions outage.
...
- execute and independently verify a provenance-test release after GitHub Actions recovers.
