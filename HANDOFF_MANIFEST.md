...
- Hosted Windows gateway lifecycle qualification: GitHub Actions run `29728466089` passed config/token ACL checks, current-user publisher access plus cross-user publisher-write denial, UTF-16 scheduled-task install/start, active health/status, atomic token rotation with graceful PID replacement, idempotent active reinstall, and process-clean uninstall. That run predates the OMP mutual-authentication patch; the updated workflow now applies and tests the pinned publisher but still requires a completed hosted run.
